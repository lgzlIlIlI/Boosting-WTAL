...
class BWA_fusion_dropout_feat_v3(torch.nn.Module):
    def __init__(self, n_feature):
        ...
        self.AE_d = nn.Sequential(
            nn.Conv1d( embed_dim,n_feature, 3, padding=1),nn.LeakyReLU(0.2),nn.Dropout(0.5) )
    ...
    def forward(self,vfeat,ffeat):
        ...
        #filter_feat = torch.sigmoid(bit_wise_attn*channel_attn)*vfeat#+vfeat
        x_atn = self.attention(filter_feat)
...
class SCmodel(nn.Module):
    ...
    def ivc_loss(self,words_logit, words_id, words_mask, hard_neg_words_logit=None, easy_neg_words_logit=None, **kwargs):
        ...
        if hard_neg_words_logit is not None:
            ...
            hard_neg_loss = torch.max(nll_loss - hard_neg_nll_loss + 0.1, tmp_0) #"beta_1": 0.1, #nll_loss hard_nll_loss,easy_nll_loss
            #这三个几乎没有区别，看看优化到最后差距在哪里
