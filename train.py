...
def criterion(outputs, labels, **args):    
    ...
    gt = np.where(labels.data.cpu().numpy() == 1)[0]
    ...
    # total loss
    total_loss_1 = (loss_mil_orig.mean()+loss_mil_supp.mean() +
...
def train(itr, dataset, args, model,model0, optimizer, rec_optimizer,rec_lr_scheduler,mask_optimizer,mask_lr_scheduler, device):
    model.train()
    ...
    total_loss1 = criterion(outputs,labels,seq_len=seq_len,device=device,opt=args,itr=itr,pairs_id=pairs_id,inputs=features)
